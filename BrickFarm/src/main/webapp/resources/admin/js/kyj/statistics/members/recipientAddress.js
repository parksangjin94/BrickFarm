...
function search() {
  getStatByRecipientAddress(params);
}

/* === ajax ==================================================================================================================== */
// 검색 버튼 핸들러에서 호출 될 api 요청 함수
function getStatByRecipientAddress(params) {
  // console.log(params);
  $.ajax({
    url: '/admin/statistics/members/recipient-address/data',
    type: 'get',
    data: params,
    dataType: 'json',
    async: false,
...
function printStatistics(data) {
  printStatByRecipientAddress(data.statByRecipientAddress);

  multipleBarChart(data.statByRecipientAddress, 'statByRecipientAddress', 'district_address');
}

// searchResult table에 매개 변수로 넘겨받은 값을 기준으로 동적 출력
function printStatByRecipientAddress(data) {
  if (data != undefined && data.length > 0) {
    let output = '';

    data.forEach(function (item, index) {
      output += '<tr>';
      output += `<td>${item.district_address}</td>`;
      output += `<td>${item.delivery_count.toLocaleString()}</td>`;
