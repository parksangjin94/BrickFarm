...
function search() {
  geStatByDayname(params);
}

/* === ajax ==================================================================================================================== */
// 검색 버튼 핸들러에서 호출 될 api 요청 함수
function geStatByDayname(params) {
  // console.log(params);
  $.ajax({
    url: '/admin/statistics/members/day/data',
    type: 'get',
    data: params,
    dataType: 'json',
    async: false,
...
function printStatistics(data) {
  printStatByDaynameTable(data.statByDayname);

  multipleBarChart(data.statByDayname, 'statByDayname', 'day');
}

// searchResult table에 매개 변수로 넘겨받은 값을 기준으로 동적 출력
function printStatByDaynameTable(data) {
  if (data != undefined && data.length > 0) {
    let output = '';

    data.forEach(function (item, index) {
      output += '<tr>';
      output += `<td>${TRANSLATED_LABEL[item.day]}</td>`;
