...
function search() {
  getStatByMemberGrade(params);
}

/* === ajax ==================================================================================================================== */
// 검색 버튼 핸들러에서 호출 될 api 요청 함수
function getStatByMemberGrade(params) {
  // console.log(params);
  $.ajax({
    url: '/admin/statistics/members/membergrade/data',
    type: 'get',
    data: params,
    dataType: 'json',
    async: false,
...
function printStatistics(data) {
  printStatByMemberGrade(data.statByMemberGrade);

  multipleBarChart(data.statByMemberGrade, 'statByMemberGrade', 'member_grade_name');
}

// searchResult table에 매개 변수로 넘겨받은 값을 기준으로 동적 출력
function printStatByMemberGrade(data) {
  if (data != undefined && data.length > 0) {
    let output = '';

    data.forEach(function (item, index) {
      output += '<tr>';
      output += `<td>${TRANSLATED_LABEL[item.member_grade_name]}</td>`;
      output += `<td>${item.member_count.toLocaleString()}</td>`;
      output += `<td>${item.total_decided_order_count.toLocaleString()}</td>`;
